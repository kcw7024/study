...
#############################SAVE & LOAD######################################
path = './_save/'
# torch.save(model.state_dict(), path + 'torch13_state_dict.pt') # save
model2.load_state_dict(torch.load(path + 'torch13_state_dict.pt')) # load
##############################################################################

#4. 평가, 예측

y_pred = (model2(x_test) >= 0.5).float()
