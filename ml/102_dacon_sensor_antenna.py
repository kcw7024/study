...
parameters = {'n_estimators': [100, 200, 300, 10, 20, 40, 50],
              ...
              # max_depth : 얕게잡을수록 좋다 너무 깊게잡을수록 과적합 우려 #None = 무한대
              'max_depth': [3, 2, 4, 6, 7, 10],
...
model = ExtraTreesRegressor(n_estimators=200, ccp_alpha=0.0,
...
# def lg_nrmse(test_y_set, preds):
#     # 각 Y Feature별 NRMSE 총합
#     # Y_01 ~ Y_08 까지 20% 가중치 부여
#     all_nrmse = []
#     for idx in range(0, 14):  # ignore 'ID'
#         rmse = mean_squared_error(
#             test_y_set[:, idx], preds[:, idx], squared=False)
#         nrmse = rmse/np.mean(np.abs(test_y_set[:, idx]))
