...
from tensorflow.python.keras.layers import Dense, Conv2D, Flatten, MaxPooling2D, Dropout, LSTM, Conv1D #이미지작업(Conv2D), 데이터를 펼친다(Flatten)
...
#2. 모델구성
model=Sequential()
# model.add(Conv2D(filters=90,kernel_size=(10,10), padding='same', input_shape=(32,32,3)))
# model.add(MaxPooling2D())
# model.add(Conv2D(140,(8,8), padding='valid', activation='relu'))
# model.add(Dropout(0.5))
# model.add(Conv2D(80,(6,6), padding='valid', activation='relu')) 
# model.add(Dropout(0.5))
# model.add(Conv2D(60,(4,4), padding='valid', activation='relu')) 
# model.add(Flatten())
...
model.add(Conv1D(100, 2, input_shape=(96, 32)))
model.add(Flatten())
model.add(Dense(100, activation='relu'))
model.add(Dense(100, activation='relu'))
model.add(Dense(100, activation='relu'))
...
model.fit(x_train,y_train, validation_split=0.2, callbacks=[earlyStopping],
          epochs=10, batch_size=218 , verbose=1)
