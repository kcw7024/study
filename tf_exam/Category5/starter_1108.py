...
def windowed_dataset(series, batch_size, n_past=40, n_future=1, shift=1):
    ...
    ds = ds.window(size=30,  # YOUR CODE HERE,
                   shift = shift,
                           drop_remainder = True)
...
def solution_model():
    ...
    model = tf.keras.models.Sequential([
      ...
      tf.keras.layers.Conv1D(32,5,1,padding="causal", input_shape=[N_PAST,1],
                            activation='relu'),
    ...
    optimizer = tf.keras.optimizers.SGD(lr=1e-2, momentum=0.9) # YOUR CODE HERE
    ...
    Loss = model.evaluate(valid_dataset)
    y_pred = model.predict(valid_dataset)
    # print(y_pred.shape) # (43153, 24, 7)
    # y_pred_list = [(x[0]) for x in y_pred]
    # y_true = []
    print("LOSS :",Loss)
    # print(np.mean(abs(valid_dataset.ravel()-y_pred.ravel())))
    # print("y_pred :",y_pred)    
    return model
